...
Namespace Hosting
    ...
    Public Structure JavaScriptRuntime
        ...
        Public Shared Function Create(attributes As JavaScriptRuntimeAttributes, version As JavaScriptRuntimeVersion, threadServiceCallback As JavaScriptThreadServiceCallback) As JavaScriptRuntime
            ...
            ThrowIfError(JsCreateRuntime(attributes, version, threadServiceCallback, handle))
            Return handle
        End Function
        ...
        Public Function CreateContext(debugApplication As IDebugApplication64) As JavaScriptContext
            Dim reference As JavaScriptContext
            If Not Environment.Is64BitProcess Then
                Throw New InvalidOperationException()
            End If
            ThrowIfError(JsCreateContext(Me, debugApplication, reference))
            Return reference
        End Function
        ...
        ''' <param name="debugApplication">The debug application to use.</param>
        ''' <returns>The created script context.</returns>
        Public Function CreateContext(debugApplication As IDebugApplication32) As JavaScriptContext
            Dim reference As JavaScriptContext
        ...
        Public Function CreateContext() As JavaScriptContext
            Dim reference As JavaScriptContext
            ThrowIfError(If(Environment.Is64BitProcess, JsCreateContext(Me, DirectCast(Nothing, IDebugApplication64), reference), JsCreateContext(Me, DirectCast(Nothing, IDebugApplication32), reference)))
            Return reference
        End Function
    End Structure
End Namespace
